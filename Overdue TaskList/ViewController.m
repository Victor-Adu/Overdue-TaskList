...
{
    NSMutableArray *taskObjectsPropertyLists = [[[NSUserDefaults standardUserDefaults] arrayForKey:TASK_OBJECTS_KEY] mutableCopy];
    
    if (!taskObjectsPropertyLists) taskObjectsPropertyLists = [[NSMutableArray alloc] init];
    [taskObjectsPropertyLists removeObjectAtIndex:indexPath.row];
    
    if(task.isCompleted == YES) task.isCompleted = NO;
    else task.isCompleted = YES;
    
    [taskObjectsPropertyLists insertObject:[self taskObjectAsAPropertyList:task] atIndex:indexPath.row];
    [[NSUserDefaults standardUserDefaults] setObject:taskObjectsPropertyLists forKey:TASK_OBJECTS_KEY];
    [[NSUserDefaults standardUserDefaults] synchronize];
...
{
    Task *taskObject = [self.taskObjects objectAtIndex:sourceIndexPath.row]; //self.taskObjects[sourceIndexPath.row];
